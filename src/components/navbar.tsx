    ...
    <nav className="navbar">
      <div className="container navbar-inner">
        ...
        <button
          ...
          aria-label={is_menu_open ? "ปิดเมนู" : "เปิดเมนู"}
        ...
        {/* เมนูนำทางหลัก */}
        <div id="primary-navigation" className={`navbar-menu-wrapper ${is_menu_open ? "open" : ""}`}>
          <ul className="navbar-links">
            {nav_links.map((link) => (
              <li key={link.href} onClick={() => setIs_menu_open(false)}>
                <Link href={link.href} aria-current={pathname === link.href ? "page" : undefined}>{link.label}</Link>
              </li>
            ))}
          </ul>
          <a
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="contact-svg-icon" style={{ marginRight: "6px" }}>
